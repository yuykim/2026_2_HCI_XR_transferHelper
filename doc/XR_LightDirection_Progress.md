...
- 그림, 마크, 안내 표식 등을 이용해 사용자가 특정 방향을 바라본 상태에서 경로를 생성하는 방식을 사용함.
...
1. 사용자가 그림, 마크, 안내 표식 등을 보고 특정 방향을 바라봄.
2. 해당 방향을 바라보고 있는 상태에서 경로를 생성함.
3. 사용자가 생성된 경로를 따라 이동함.
4. 이동 중간에 다시 기준 마크가 나오면, 해당 마크를 기준으로 경로를 수정하거나 다시 정렬할 수 있음.
5. 사용자가 경로를 벗어나면 텍스트와 경고음으로 알림을 제공함.
6. 사용자가 목적지에 도착하면 도착 안내 텍스트와 알림음을 제공함.
...
- [ ] 수업 강의실에서 교수님 오피스까지 이동하는 경로
- [ ] 수업 강의실에서 산학협력관까지 이동하는 경로
...
- [ ] 남은 거리 또는 예상 도착 정보
- [ ] 경로 안내 상태 표시
- [ ] 경로 이탈, 재정렬 가능 상태, 도착 상태를 더 명확하게 표시
...
- [ ] 중간 기준 마크를 발견했을 때 경로를 다시 정렬하는 흐름 검토

### 시각 요소 개선

- [ ] 현재 원뿔과 원통을 조합한 화살표를 더 명확한 방향 안내 오브젝트로 개선
- [ ] 화살표 크기, 방향, 색상, 배치를 조정해 사용자가 진행 방향을 더 쉽게 인식하도록 수정
- [ ] 루트 색상을 기존 노란색에서 더 눈에 띄는 네온 계열 색상으로 변경
- [ ] 경로선과 화살표가 실제 공간 위에서 잘 보이도록 밝기와 대비 조정
...
- 실제 사용할 경로 데이터 2종을 제작하고, UI에 필요한 안내 정보를 추가함.
- 화살표와 경로선 색상을 개선해 방향 안내가 더 명확하게 보이도록 정리함.
